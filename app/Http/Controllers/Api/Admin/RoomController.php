...
class RoomController extends Controller
{
    ...
    public function store(Request $request){
        if( strlen( $request->name ) <= 0 ){
            return response()->json([
                'ok' => false ,
                'message' => 'សូមបញ្ជាក់ឈ្មោះរបស់សាល'
            ],403);
        }
        $room = RecordModel::where('name', $request->name )->first();
        if( $room != null ){
            return response()->json([
                'ok' => false ,
    ...
    public function update(Request $request){
        ...
        $room = RecordModel::where('name', $request->name )
        ->where('id','!=',$record->id)
        ->first();
        if( $room != null ){
            return response()->json([
    ...
    public function pdf(Request $request)
    {
        ...
        if($record) {
            ...
            if( !\Storage::disk('data')->exists( $moduleFolder )){
                return response()->json([
                    'ok' => false ,
                    'message' => "មិនមានឯកសារនេះឡើយ។ {$moduleFolder}"
                ],500);
            }
